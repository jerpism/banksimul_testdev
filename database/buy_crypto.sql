...
buy_crypto:BEGIN
 ...
 --   SELECT account.id_account INTO @accid FROM card JOIN account ON card.id_account = account.id_account WHERE id_card = cardid;
    ...
    IF(@test > 0) THEN
		UPDATE cryptoaccount SET balance = balance+amount*0.005 WHERE id_cryptoaccount = id_crypto;
        ...
        IF(@test > 0) THEN
            ...
            INSERT INTO cryptoaccount_action (id_cryptoaccount,amount,date,action_type) VALUES(id_crypto,+amount*0.005,NOW(),'Kryptovaluutan osto');
		ELSE
			ROLLBACK;
            END IF;
		ELSE
