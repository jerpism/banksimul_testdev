...
sell_crypto:BEGIN
 ...
 --   SELECT account.id_account INTO @accid FROM card JOIN account ON card.id_account = account.id_account WHERE id_card = cardid;
    ...
    IF(@test > 0) THEN
		UPDATE account SET balance = balance+amount/@rate WHERE id_account = id_acc;
        
        SET @test=row_count();
        
        IF(@test > 0) THEN
			COMMIT;
            INSERT INTO account_action(id_account,amount,date,action_type) VALUES(id_acc,+amount/@rate,NOW(),'Kryptovaluutan myynti');
